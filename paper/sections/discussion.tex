...
The central finding of this study is that near-ceiling classification accuracy on
a strongly structured perturbation dataset does not indicate equivalence among
...
The comparison with related single-cell GNN methods is informative.
...
domains where the heterogeneous graph topology provides information unavailable
to PCA.
...
state classification, but reinforces the principle that typed gene interaction
graphs recover perturbation effects that linear encodings miss.
...
The multitask HGT jointly encodes p53 status, CENP-A status, and six-condition
perturbation state in a single embedding, a property absent from single-task
linear classifiers.
